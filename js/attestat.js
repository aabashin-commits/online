...
(() => {
  ...
  const classSize = root.dataset.classSize || '15';
  ...
  const tabs = [
    ...
    {
      pill: 'До {size} учеников в классе',
      title: 'Мини-группы',
      ...
      bubbles: [
        { text: 'до {size} учеников', style: 'light' },
        { text: 'внимание каждому', style: 'solid' },
      ],
    },
    {
      pill: 'Прогресс под контролем',
      title: 'Родители видят успеваемость ребёнка по каждой теме',
      subtitle: 'Вы знаете, что уже получается, а над чем ещё нужно поработать',
      lead: '**На каждом уроке педагог отмечает прогресс ребёнка в электронном журнале.** Раз в пять недель родители получают подробную обратную связь по всем предметам, дисциплине, общению с одноклассниками и достижениям.',
      image: 'assets/img/attestat/dz.webp',
      figura: 'assets/img/attestat/diplom_figura.webp',
      alt: 'Улыбающаяся ученица за партой в классе',
      bubbles: [
        { text: 'Подробная обратная связь', style: 'light' },
        { text: 'Каждые 5 недель', style: 'solid' },
      ],
    },
    {
      pill: 'Удобный формат',
      title: 'Заочная или семейная форма обучения',
      subtitle: 'Выберите подходящий вариант оформления ребёнка в школе',
      lead: 'При заочной форме ребёнок числится в школе-партнёре, получает текущие оценки и справки об обучении. При семейной форме аттестация проходит раз в год на платформе. Промежуточная аттестация проводится дистанционно, а аттестат государственного образца выдаёт школа-партнёр.',
    ...
    {
      pill: 'Уроки в записи',
      title: 'Все уроки можно посмотреть в записи',
      subtitle: 'Ребёнок сможет вернуться к пропущенной теме и выполнить задания',
      lead: 'Если ребёнок пропустил занятие, он может посмотреть его в записи. Если возникают сложности с учёбой, педагог определяет пробелы и помогает их закрыть: предлагает дополнительные занятия или индивидуальные домашние задания с проверкой.',
      image: 'assets/img/attestat/stress.webp',
      figura: 'assets/img/attestat/diplom_figura.webp',
      alt: 'Группа улыбающихся учеников',
      bubbles: [
        { text: 'Все уроки в записи', style: 'light' },
        { text: 'Помощь с пробелами', style: 'solid' },
      ],
    },
  ];
